...
def register_extras() -> None:
    ...
    _register("aiku", [
        Ability("aiku_p1", "aiku", "passive", 1, "Board Vision",
            "\u300cChess, not football.\u300d",
            "+1 defense on every duel \u2014 the captain reads three moves ahead.",
            dfd=lambda c: 1),
        Ability("aiku_s1", "aiku", "skill", 1, "Frozen Line",
            "\u300cYou were never getting past.\u300d",
            "Once per match: his standing tackle erases the dribble \u2014 no dice, no way through.",
            auto="stop"),
        Ability("aiku_p2", "aiku", "passive", 2, "Clean Sheet Greed",
            "\u300cProtect what\u2019s mine.\u300d",
            "+2 defense while his team is ahead \u2014 nobody scores on his watch.",
            dfd=lambda c: 2 if c["diff"] > 0 else 0),
        Ability("aiku_s2", "aiku", "skill", 3, "Box Lockdown",
            "\u300cStep into my box and you\u2019re already late.\u300d",
            "Once per match: defending a shot, +3 to his read \u2014 the door stays shut.",
            dfd=lambda c: 3, when=lambda c: c["action"] == "shoot"),
        Ability("aiku_p3", "aiku", "passive", 3, "Captain\u2019s Gravity",
            "\u300cLevel scores. My rules.\u300d",
            "+1 to ALL duel powers while the score is dead level.",
            att=lambda c: 1 if c["diff"] == 0 else 0,
            dfd=lambda c: 1 if c["diff"] == 0 else 0),
        Ability("aiku_s3", "aiku", "skill", 4, "Absolute Lock",
            "\u300cThe game ends where I decide.\u300d",
            "ULTIMATE: on his defensive stand his die never drops below 4, and +2 defense.",
            dfd=lambda c: 2, die_floor=4),
    ])
    _register("ness", [
        Ability("ness_p1", "ness", "passive", 1, "Supply Line",
            "\u300cHe eats. I serve. Simple.\u300d",
            "Completed passes grant their receiver +1 on the next action.",
            pass_buff=1),
        Ability("ness_s1", "ness", "skill", 1, "Golden Ball",
            "\u300cOne look, one pass.\u300d",
            "Once per match: his through ball sends the receiver surging +1 zone.",
            pass_advance=1),
        Ability("ness_p2", "ness", "passive", 2, "Loyal Engine",
            "\u300cBeside the Emperor, I never slow.\u300d",
            "+1 to ALL duel powers while Kaiser shares the pitch.",
            att=lambda c: 1 if _mate("kaiser") else 0,
            dfd=lambda c: 1 if _mate("kaiser") else 0),
        Ability("ness_s2", "ness", "skill", 3, "Set-Piece Darling",
            "\u300cGive me the dead ball.\u300d",
            "Once per match, on his penalty: nerve and precision \u2014 +2 on the nerve duel.",
            pen_edge=2),
        Ability("ness_p3", "ness", "passive", 3, "Emperor\u2019s Shadow",
            "\u300cEvery light casts one.\u300d",
            "Completed passes grant receivers +2, and +1 defense on his marks.",
            pass_buff=2, dfd=lambda c: 1),
        Ability("ness_s3", "ness", "skill", 4, "Forever Number 10",
            "\u300cThis ball is his destiny \u2014 I only deliver it.\u300d",
            "ULTIMATE: his pass cannot be intercepted, and the receiver\u2019s next action gains +4.",
            pass_advance=1, pass_buff=4),
    ])
    _register("charles", [
        Ability("charles_p1", "charles", "passive", 1, "Royal Weight",
            "\u300cThe ball obeys bloodline.\u300d",
            "+1 on his deliveries \u2014 passes carve the field open.",
            att=lambda c: 1 if c["action"] == "pass" else 0),
        Ability("charles_s1", "charles", "skill", 1, "Switch of Play",
            "\u300cOne switch, whole new game.\u300d",
            "Once per match: a forty-meter switch \u2014 the receiver breaks +2 zones.",
            pass_advance=2),
        Ability("charles_p2", "charles", "passive", 2, "Prodigy\u2019s Touch",
            "\u300cSkills run in my family.\u300d",
            "+2 on his dribble feints.",
            att=lambda c: 2 if c["action"] == "dribble" else 0),
        Ability("charles_s2", "charles", "skill", 3, "The Tease",
            "\u300cTry to take it. I dare you.\u300d",
            "Once per match: stop his dribble and the referee spots the foul \u2014 free kick.",
            on_lost="foul"),
        Ability("charles_p3", "charles", "passive", 3, "Charm Game",
            "\u300cEyes up \u2014 they\u2019re always open.\u300d",
            "+1 on his passes, and completed passes grant receivers +1.",
            att=lambda c: 1 if c["action"] == "pass" else 0, pass_buff=1),
        Ability("charles_s3", "charles", "skill", 4, "Trance Pass",
            "\u300cThe pass writes the goal before it happens.\u300d",
            "ULTIMATE: the pass splits the line for +2 zones, and the receiver\u2019s next action gains +2.",
            pass_advance=2, pass_buff=2),
    ])
    _register("zantetsu", [
        Ability("zantetsu_p1", "zantetsu", "passive", 1, "Never Stalling",
            "\u300cSpeed is a habit.\u300d",
            "His die never shows less than 2 \u2014 momentum never dies.",
            die_floor=2),
        Ability("zantetsu_s1", "zantetsu", "skill", 1, "Steel Dash",
            "\u300cBlink and I\u2019m gone.\u300d",
            "Once per match: the burst beats his marker without rolling.",
            auto="win", when=lambda c: c["action"] == "dribble"),
        Ability("zantetsu_p2", "zantetsu", "passive", 2, "Blade\u2019s Edge",
            "\u300cFirst step, last word.\u300d",
            "+1 when he carries or strikes.",
            att=lambda c: 1 if c["action"] in ("dribble", "shoot") else 0),
        Ability("zantetsu_s2", "zantetsu", "skill", 3, "Iron Run",
            "\u300cThe lane is always there.\u300d",
            "Once per match: two zones carved in a single burst.",
            auto="win", zone_extra=2, when=lambda c: c["action"] == "dribble"),
        Ability("zantetsu_p3", "zantetsu", "passive", 3, "Afterimage",
            "\u300cYou\u2019re chasing the ghost already.\u300d",
            "+2 on his dashes and +1 defense \u2014 speed is his shield.",
            att=lambda c: 2 if c["action"] == "dribble" else 0, dfd=lambda c: 1),
        Ability("zantetsu_s3", "zantetsu", "skill", 4, "Unreachable",
            "\u300cCatch me first.\u300d",
            "ULTIMATE: his darting strike hits with +2 power while the keeper reads it \u22123.",
